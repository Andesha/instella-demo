...
Set `MODEL_ID` before download and inference to select another model. For actual fine-tuning, use `Base` as the conventional starting point; the included mock training starts from scratch because its purpose is only to exercise AMD's official training stack.
